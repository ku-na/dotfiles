...
" Updated: Jonas Juselius <jonas@iki.fi>
...
setlocal indentkeys+==~module,=~type,=~interface,=~program,=~subroutine
...
function FortranGetIndent(lnum)
  ...
  "Add a shiftwidth to statements following module, type, interface
  " program, subroutine, function
  if prevstat =~? '^\s*\(\d\+\s\)\=\s*\(else\|case\|where\|elsewhere\)\>'
	\ ||prevstat =~? '^\s*\(\d\+\s\)\=\s*\(program\|type\|interface\)\>'
	\ ||prevstat =~? '^\s*\(\d\+\s\)\=\s*\(module\|program\|type\|interface\)\>'
	\ ||prevstat =~? '^\s*\(\d\+\s\)\=\s*\(subroutine\|function\|forall\)\>'
    ...
    if prevstat =~? '\<module\>' && prevstat =~? '\<procedure\>'
      let ind = ind - &sw
    endif
    if prevstat =~? '\<type\>' && prevstat =~? '::'
      let ind = ind - &sw
    endif
  endif
  ...
  if getline(v:lnum) =~? '^\s*\(\d\+\s\)\=\s*'
	\. '\(else\|elsewhere\|case\|end\s*\(if\|where\|select\|forall\|interface\|module\|program\|subroutine\|function\|type\)\)\>'
    let ind = ind - &sw
